...
- **Push notifications built end-to-end**: managers get “Now pouring” / “Line finished” pushed to their phone, even with the app closed, when a beer advances or is finished. App side (subscribe/unsubscribe UI under More → Notifications, `sendCellarPush` hooked into `advance`/`finishAndChoose`/`doSwap`) is in App.jsx. Repo side is NOT yet done by Kyle: `sw.js` (service worker, → `public/`), `notify.js` (send endpoint, → `api/`), updated `package.json` (adds `web-push`), and a Supabase table via `push-setup.sql`, plus two Vercel env vars (`VAPID_PRIVATE_KEY`, `SUPABASE_SERVICE_ROLE_KEY`). Full walkthrough in `SETUP-NOTIFICATIONS.md`. **Check with Kyle whether this setup is done before assuming notifications work.**
...
- **Confirm the notification system’s repo/Supabase/Vercel setup is actually done** (see SETUP-NOTIFICATIONS.md) before assuming pushes work. This is the one outstanding multi-file, non-App.jsx piece of work.
- Confirm the PWA files (manifest.json, icon-192.png, icon-512.png, index.html lines) were actually added to the repo — asked for, not confirmed done.
